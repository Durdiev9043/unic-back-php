...
//        return $this->sendSuccess($atvet,'Viloyatlar kesimida qilingan ishlar');
    ...
    public function district($id){
        ...
        $atvet[$district->name]['kpi']= (double)number_format($kpi,1);
...
//        $atvet['name']=$tasks->user->name;
    foreach ($tasks as $task){
        ...
        $atvet[$task->id]['day']=$task->created_at->addMinutes(300)->format('d.m.Y  H:i');
    }
...
//        $atvet[$item->id]['user'] = User::where('id',$item->user_id)->first()->name;
        $atvet[$item->id]['region'] = table('regions')->select('id','name')->where('id',$user->region_id)->first()->name;
